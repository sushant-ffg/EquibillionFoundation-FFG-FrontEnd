          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                {(onEdit || onDelete || onView || viewPath) && <th scope="col" className="relative px-6 py-3">
                    <div className="h-4 bg-gray-200 rounded animate-pulse"></div>
                  </th>}
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              length: 5
                  ...
                  {(onEdit || onDelete || onView || viewPath) && <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="h-4 bg-gray-200 rounded animate-pulse"></div>
                    </td>}
                </tr>)}
            </tbody>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {columns.map((column, index) => <th key={index} scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  {column.sortable ? <button className="group flex items-center space-x-1 focus:outline-none" onClick={() => typeof column.accessor === 'string' ? handleSort(column.accessor) : null}>
                      <span>{column.header}</span>
                      <span className="flex-shrink-0">
                        {sortField === column.accessor ? sortDirection === 'asc' ? <ChevronUp className="h-4 w-4 text-gray-500" /> : <ChevronDown className="h-4 w-4 text-gray-500" /> : <ChevronDown className="h-4 w-4 text-gray-300 group-hover:text-gray-500" />}
                      </span>
